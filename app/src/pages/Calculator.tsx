...
export function Calculator() {
  ...
  const handleCalculate = async (endpoint: 'solve' | 'integrate' | 'derive') => {
    setLoading(true);
    setError(null);
    setResult(null);

    try {
      const response = await fetch(`http://localhost:8000/api/calculator/${endpoint}`, {
        method: 'POST',
      ...
      if (data.solution) {
        setResult(`x = ${JSON.stringify(data.solution)}`);
      } else if (data.result) {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-900 text-gray-100 font-sans p-6 overflow-y-auto">
        <header className="mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">Calculadora Científica</h1>
          <p className="text-gray-400">Resuelve ecuaciones, derivadas e integrales.</p>
        </header>

        <div className="max-w-3xl bg-gray-800 rounded-xl border border-gray-700 shadow-2xl p-6">
