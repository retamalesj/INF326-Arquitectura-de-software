 ...
 * POST /v1/channels/
 */
export const createChannel = async (body: CreateChannelDTO): Promise<Channel | null> => {
  try {
 ...
 * GET /v1/channels/
 */
export const getChannels = async (page = 1, page_size = 10): Promise<Channel[] | null> => {
  try {
    const res = await fetch(`${API_GATEWAY_URL}/channels?page=${page}&page_size=${page_size}`)
 ...
 */
export const getChannelById = async (channel_id: string): Promise<Channel | null> => {
  try {
 ...
 */
export const updateChannel = async (channel_id: string, body: UpdateChannelDTO): Promise<Channel | null> => {
  try {
    const res = await fetch(`${API_GATEWAY_URL}/channels/${channel_id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
 ...
 */
export const deleteChannel = async (channel_id: string): Promise<{ id: string; is_active: boolean } | null> => {
  try {
    const res = await fetch(`${API_GATEWAY_URL}/channels/${channel_id}`, { method: 'DELETE' })
 ...
 */
export const reactivateChannel = async (channel_id: string): Promise<{ id: string; is_active: boolean } | null> => {
  try {
    const res = await fetch(`${API_GATEWAY_URL}/channels/${channel_id}/reactivate`, { method: 'POST' })
 ...
 */
export const getChannelBasic = async (channel_id: string): Promise<Channel | null> => {
  try {
 ...
 */
export const getChannelStatus = async (channel_id: string): Promise<{ id: string; is_active: boolean } | null> => {
  try {
    const res = await fetch(`${API_GATEWAY_URL}/channels/${channel_id}/status`)
    if (!res.ok) throw new Error(`Error obteniendo status del canal: ${res.status}`)
    return await res.json()
  } catch (err) {
    console.error(err)
    return null
